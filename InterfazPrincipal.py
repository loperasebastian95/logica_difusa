...
class LoginPage(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        main_frame = tk.Frame(self, bg="#41cac6", height=431, width=626)  # this is the background
        ...
        frame_login = tk.Frame(main_frame, bg="#4c98cf", relief="groove", bd=2)  # this is the frame that holds all the login details and buttons
        frame_login.place(rely=0.30, relx=0.10, height=130, width=500)
        button = ttk.Button(frame_login, text="Crear Rubricas",command=lambda: abrirCreacionRubricas())
        button.place(rely=0.70, relx=0.10)

        signup_btn = ttk.Button(frame_login, text="Cargar calificaciones", command=lambda: get_signup())
        signup_btn.place(rely=0.70, relx=0.32)

        cargar_btn = ttk.Button(frame_login, text="Editar Rubricas/Materias", command=lambda: get_signup())
        cargar_btn.place(rely=0.70, relx=0.60)

        def abrirCreacionRubricas():
            PaginaRubricas()

        def abrirCargarCalificaciones():
            pass
        
        def abrirEdicionRubricas():
            pass
...
class PaginaRubricas(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        label_reglas =[]
        inputs_reglas=[]
        main_frame = tk.Frame(self, bg="#4c98cf", height=431, width=800)
        # pack_propagate prevents the window resizing to match the widgets
        main_frame.pack_propagate(0)
        main_frame.pack(fill="both", expand="true")

        self.geometry("900x431")
        self.resizable(0, 0)

        self.title("Creacion De Rubricas")

        text_styles = {"font": ("Verdana", 10),
                       "background": "#3F6BAA",
                       "foreground": "#E1FFFF"}

        label_nombre_rubrica = tk.Label(main_frame, text_styles, text="Nombre De La Materia")
        label_nombre_rubrica.grid(row=1, column=0)

        input_materia = tk.Button(main_frame, width=20, cursor="xterm")
        input_materia.grid(row=1, column=1)

        label_regla = tk.Label(main_frame, text_styles, text="Regla: "+str(len(inputs_reglas)+1))
        label_regla.grid(row=3, column=0)

        input_regla1 = tk.Button(main_frame, width=20, cursor="xterm")
        input_regla1.grid(row=3, column=1)

        # Create a StringVar to hold the selected option
        selected_option = tk.StringVar()

        # Create a list of choices
        choices = ["Entre", "igual", "menor", "mayor","menor o igual","mayor o igual"]

        # Create the OptionMenu widget
        option_menu = tk.OptionMenu(main_frame, selected_option, *choices)
        option_menu.grid(row=3, column=2)

        input_regla1 = tk.Button(main_frame, width=20, cursor="xterm")
        input_regla1.grid(row=3, column=3)
        

        input_materia = tk.Button(main_frame, width=20, cursor="xterm")
        input_materia.grid(row=3, column=4)

        agregar_btn = ttk.Button(main_frame, text="Agregar Rubrica +", command=lambda: agregarRubrica())
        agregar_btn.grid(row=3, column=5)

        eliminar_btn = ttk.Button(main_frame, text="Eliminar Rubrica -", command=lambda: eliminarRubrica())
        eliminar_btn.grid(row=3, column=6)

        button = ttk.Button(main_frame, text="Crear Reglas", command=lambda: signup())
        button.grid(row=4, column=5)

        def agregarRubrica():
            label_input = tk.Label(main_frame, text_styles, text="Nombre Rubrica"+": "+str(len(label_inputs)+1))
            label_input.grid(row=len(label_inputs) + 1,column=0)
            input = tk.input(main_frame,width=20, cursor="xterm")
            input.grid(row=len(input_inputs)+1,column=1)
            label_input_regla = tk.Label(main_frame, text_styles, text="Nombre Rubrica"+": "+str(len(label_inputs)+1))
            label_input.grid(row=len(label_inputs) + 1,column=0)
            input_inputs.append(input)
            label_inputs.append(label_input)
            print(str(len(input_inputs)))

        def eliminarRubrica():
            if input_inputs:
                input = input_inputs.pop()
                input.grid_forget()
                label_input = label_inputs.pop()
                label_input.grid_forget()

        def signup():
            # Creates a text file with the Username and password
            user = input_materia.get()
            pw = input_pw.get()
...
def on_closing():
    # Función que se ejecutará al cerrar la ventana de tkinter
    print("Cerrando la aplicación...")
    # Agrega aquí cualquier limpieza adicional que necesites realizar antes de cerrar el programa.
    os._exit(0)  # Mata el proceso
...
# Asegurarse de que la función on_closing se ejecute al finalizar
